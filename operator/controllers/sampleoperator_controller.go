...
import (
	...
	appv1alpha1 "github.com/okd-project/sample-operator/api/v1alpha1"
)
...
// the SampleOperator object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.14.1/pkg/reconcile
func (r *SampleOperatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx)

	// fetch the SampleOperator instance
	instance := &appv1alpha1.SampleOperator{}
	...
	if !reflect.DeepEqual(pod.Status, status) {
		pod.Status = status
...
func (r *SampleOperatorReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.SampleOperator{}).
		Owns(&corev1.Pod{}).
		Complete(r)
}
